...
MARKER="# === Adobe Block List (macutil) ==="

blockAdobeHosts() {
    printf "%b\n" "${YELLOW}Applying Adobe URL Block List to /etc/hosts...${RC}"

    if grep -qF "$MARKER" /etc/hosts 2>/dev/null; then
        printf "%b\n" "${GREEN}Adobe block list is already applied.${RC}"
        exit 0
    fi

    TEMP_HOSTS=$(mktemp /tmp/adobe_hosts_XXXXXX)

    cat > "$TEMP_HOSTS" << 'ADOBE_HOSTS_EOF'
# === Adobe Block List (macutil) ===
0.0.0.0 adobe.io
...
0.0.0.0 resources.licenses.adobe.com
...
0.0.0.0 23.22.254.206
0.0.0.0 3.209.122.138
0.0.0.0 18.154.7.93
0.0.0.0 13.35.238.103
0.0.0.0 54.144.73.197
0.0.0.0 13.35.238.72
0.0.0.0 13.35.238.108
...
ADOBE_HOSTS_EOF
    ...
    ENTRY_COUNT=$(grep -c "^0\.0\.0\.0" "$TEMP_HOSTS" || true)
    printf "%b\n" "${CYAN}Adding $ENTRY_COUNT Adobe domains/IPs to /etc/hosts...${RC}"

    $ESCALATION_TOOL tee -a /etc/hosts < "$TEMP_HOSTS" > /dev/null
    rm -f "$TEMP_HOSTS"
    ...
    printf "%b\n" "${GREEN}Done! $ENTRY_COUNT Adobe domains/IPs are now blocked in /etc/hosts.${RC}"
    printf "%b\n" "${YELLOW}To remove: delete all lines starting from '$MARKER' in /etc/hosts.${RC}"
}
